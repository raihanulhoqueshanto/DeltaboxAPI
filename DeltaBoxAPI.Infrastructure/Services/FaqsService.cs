...
namespace DeltaboxAPI.Infrastructure.Services
{
    public class FaqsService : IFaqsService
    {
        ...
        public async Task<PagedList<ProductReviewVM>> GetProductReview(GetProductReview request)
        {
            string conditionClause = " ";
            var queryBuilder = new StringBuilder();
            var parameter = new DynamicParameters();
            ...
            queryBuilder.AppendLine("SELECT product_review.*, count(*) over() as TotalItems FROM product_review ");
            ...
            if (!string.IsNullOrEmpty(request.Keyword))
            {
                queryBuilder.AppendLine($"{Helper.GetSqlCondition(conditionClause, "AND")} (customer_name LIKE @Keyword OR customer_email LIKE @Keyword OR review LIKE @Keyword)");
                conditionClause = " WHERE ";
                parameter.Add("Keyword", $"%{request.Keyword}%", DbType.String, ParameterDirection.Input);
            }
            ...
            // Optional: Add order by to sort reviews (e.g., by rating or date)
            queryBuilder.AppendLine("ORDER BY rating DESC, id DESC");
            ...
            string query = queryBuilder.ToString();
            var result = await _mysqlContext.GetPagedListAsync<ProductReviewVM>(request.CurrentPage, request.ItemsPerPage, query, parameter);
            return result;
        }
    }
}
